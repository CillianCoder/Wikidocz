...
// [hero_content] — Category tag with per-category bg + text colors
add_shortcode( 'hero_content', function( $atts ) {
    ...
    if ( ! $query->have_posts() ) return '<p style="color:#C8D4E3">No articles yet.</p>';
    $query->the_post();
...
// [hero_image] — Featured image or fallback gradient
add_shortcode( 'hero_image', function( $atts ) {
    ...
    $args = $a['post_id']
        ...
        : [ 'posts_per_page' => 1, 'orderby' => 'date', 'order' => 'DESC' ];
    $query = new WP_Query( $args );
    if ( ! $query->have_posts() ) return '<div class="hero-image-fallback">Featured image</div>';
    $query->the_post();
